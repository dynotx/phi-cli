...
NOTE: PyRosetta installed from quarterly wheels (more stable than pyrosettacolabsetup).
If installation fails, BindCraft continues without Rosetta scoring features.
Core AlphaFold-based design functionality will work either way.
"""
...
# Try PyRosetta via quarterly wheels (Path 2 from Rosetta team)
# Fallback gracefully if it fails
PYROSETTA_WHEEL_URL = (
...
image = (
    ...
    # Install PyRosetta from quarterly wheels (non-fatal)
    .run_commands(
        f"python -m pip install pyrosetta --find-links {PYROSETTA_WHEEL_URL} "
        "|| echo '⚠ PyRosetta install failed. Continuing without it - some scoring features unavailable.'"
    )
    .pip_install(
...
def bindcraft(
    ...
    if filter_option == "Default":
        ...
        filter_settings_path = "/root/bindcraft/settings_filters/peptide_relaxed_filters.json"
        ...
        if not trajectory_exists:
            ...
            if trajectory_metrics["terminate"] == "":
                ...
                if advanced_settings["enable_mpnn"]:
                    ...
                    restricted_AAs = (
                        {aa.strip().upper() for aa in advanced_settings["omit_AAs"].split(",")}
                    ...
                    if mpnn_sequences:
                        ...
                        for mpnn_sequence in mpnn_sequences:
                            ...
                            highest_plddt_key = int(
                                max(plddt_values, key=lambda x: plddt_values.get(x, 0))
                            )
    ...
    if upload_to_gcs and gcs_bucket and run_id:
        ...
        gcs_prefix = f"runs/{run_id}/bindcraft"
        output_files = []
        ...
        return {
            ...
            "message": f"BindCraft completed: {len(output_files)} files uploaded to GCS",
            "metadata": {
                "binder_name": binder_name,
                "target_chains": chains,
                "number_of_designs": len([f for f in output_files if f["artifact_type"] == "pdb"]),
            },
        }
    else:
...
def main(
    ...
    Parameter names match BindCraft's actual interface from:
    https://github.com/martinpacesa/BindCraft

    Args:
        starting_pdb (str): Path to the input PDB file (matches BindCraft arg name).
        chains (str, optional): Target chain(s) in the PDB. Defaults to "A".
        target_hotspot_residues (str, optional): Hotspot residues on the target.
            Examples: "1,2-10" or chain specific "A1-10,B1-20" or entire chains "A".
            If left blank, AF2 will select binding site automatically.
            Defaults to "".
        lengths (str, optional): Comma-separated min,max lengths for binder (e.g., "50,130").
            Defaults to "50,130".
        number_of_final_designs (int, optional): How many designs passing all filters to aim for.
            Defaults to 1.
        max_trajectories (int | None, optional): Maximum number of design trajectories to run.
            Defaults to None.
        binder_name (str | None, optional): What to prefix your designed binder files with.
            If None, derived from input PDB filename. Defaults to None.
        design_protocol (str, optional): Protocol: "Default", "Beta-sheet", or "Peptide".
            Defaults to "Default".
        out_dir (str, optional): Directory to save the output files. Defaults to "./out/bindcraft".
        run_name (str | None, optional): Optional name for the run subdirectory.
            If None, uses timestamp. Defaults to None.
    ...
    pdb_str = open(starting_pdb).read()
    binder_name = binder_name or Path(starting_pdb).stem
