...
def _split_pdb_into_protein_and_ligand(pdb_name: str) -> tuple[Any, Any]:
    """
...
def _process_ligand(ligand, res_name, chain, ligand_smiles=None):
    ...
    if sub_mol is None:
        warn(f"sub_mol is None for {res_name}", stacklevel=2)
        return None, None
...
def _write_sel_to_pdb(protein: Any, out_pdb_file: str) -> str:
    """
...
def _write_mol_to_sdf(rd_mol: Any, out_sdf_file: str) -> str:
    """
...
def extract_ligand(
    ...
    if out_sdf_file is None:
        out_sdf_file = (
            str(Path(out_pdb_file).with_suffix("")) + f"_{ligand_name}_{ligand_chain}.sdf"
        )
...
def extract_all_ligands(pdb_name, out_pdb_name):
    ...
    all_ligand_names_chains = set(
        zip(ligand_sel.getResnames(), ligand_sel.getChids(), strict=False)
    )
...
if __name__ == "__main__":
    ...
    for i, ligand_name in enumerate(args_ligand_names):
        ...
        outputs.append(
            extract_ligand(args.pdb_id, ligand_name, chain, f"{args.pdb_id}_protein.pdb")
        )
    print(outputs)
